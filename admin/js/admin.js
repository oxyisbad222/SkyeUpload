document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderStatus = async () => {
        try {
            ...
            // Only update the DOM if the status tab is visible
            ...
            const torrentsHtml = status.torrents.map(t => `
                <div class="bg-gray-700 p-4 rounded-md">
                    ...
                    <div class="w-full bg-gray-600 rounded-full h-2.5 mt-2">
                        <div class="bg-indigo-500 h-2.5 rounded-full" style="width: ${t.progress}%"></div>
                    </div>
    ...
    content.addEventListener('click', async (e) => {
        ...
        if (button.classList.contains('delete-btn')) {
            ...
            renderRequests(); // Re-render to show status change
        }
    });
