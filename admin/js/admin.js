document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderManageContent = async () => {
        ...
        try {
            ...
            const contentHtml = allContent.map(item => `
                <tr class="border-b border-gray-700" data-id="${item.id}" data-type="${item.mediaType}">
                    <td class="p-4 flex items-center">
                        <img src="${item.poster_path ? 'https://image.tmdb.org/t/p/w92' + item.poster_path : 'https://placehold.co/40x60/1f2937/9ca3af?text=N/A'}" class="w-10 h-15 rounded-md mr-4" alt="poster">
                        <span>${item.title}</span>
                    </td>
                    <td class="p-4 text-gray-400 capitalize">${item.streamType}</td>
                    <td class="p-4 text-right">
                        <button class="delete-media-btn text-red-400 hover:text-red-300">Delete</button>
                    </td>
                </tr>
            `).join('');
            ...
            content.innerHTML = `<div class="bg-red-900 p-4 rounded-lg">${error.message}</div>`;
        }
    };
